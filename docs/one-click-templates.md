...
| **Easypanel** | [easypanel-io/templates#1477](https://github.com/easypanel-io/templates/pull/1477) | `templates/appboard/*` (index.ts, meta.yaml, logo, screenshot) | OPEN — deploy-preview ✅; review Ahson-Shaikh: smoke test + pinowane tagi + linki → **poprawione** (patrz niżej) | pinowane (`0.8.0` backend / `0.8.2` panel) |
...
1. **Smoke test padał** — backend startował przed Postgresem i crash-loopował na
   ...
   `0.8.0`: `runMigrations()` rozpoznaje błędy połączenia i ponawia migracje
...
2. **"The latest tags has to go away"** — szablon Easypanel pinuje teraz
   `appboard-backend:0.8.0` + `appboard-web:0.8.2` (workflow publikuje tag
   wersji obok `latest`).
...
- tagi: `latest`, wersja z `package.json` (np. `0.8.0`), pełny SHA commita
