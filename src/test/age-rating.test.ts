...
describe("Age Rating module", () => {
	...
	it("GET /api/age-rating-presets returns all presets", async () => {
		const response = await app
			.handle(new Request("http://localhost/api/age-rating-presets"))
			.then((res) => res.json());
	...
	it("GET /api/apps/:appId/age-rating returns null when none exists", async () => {
		const response = await app
			.handle(new Request(`http://localhost/api/apps/${appId}/age-rating`))
			.then((res) => res.json());
	...
	it("PUT with preset 'everyone' creates rating", async () => {
		const res = await app.handle(
			new Request(`http://localhost/api/apps/${appId}/age-rating`, {
	...
	it("GET with invalid appId returns 422", async () => {
		const res = await app.handle(
			new Request("http://localhost/api/apps/not-a-uuid/age-rating"),
		);
