...
- **Ingest**: `POST /api/public/aso-reports` (PRE-auth-guard, jak feedback; rate limit 100/h/IP - jeden check-up "all markets" wysyla obserwacje per storefront, IP tylko jako sha256). Tabele `public_aso_reports` + `public_keyword_observations` - crowd data, source="web_client", NIEZAUFANE (walidacja zakresow na wejsciu) i trzymane OSOBNO od danych workspace'ow. To rosnaca baza keyword->score per kraj/dzien.
- **Udostepnianie raportu** (od 0.17.0): przegladarka po skonczonym check-upie wysyla CALY snapshot raportu do `POST /api/public/aso-reports/share` (`{ tool, store, country: "all"|cc, trackId?, appName?, payload: { version, ... } }`, limit 4 MB, 20/h/IP) i dostaje `{ id }`; `GET /api/public/aso-reports/share/:id` (300/h/IP, cache 5 min) oddaje snapshot + metadane (bez ipHash). Tabela `public_report_shares`, retencja 365 dni (`PublicReportsService.cleanupShares` w sprzataniu schedulera). Payload jest formatem panelu (`src/lib/aso-check/share.ts`), backend sprawdza tylko koperte - panel renderuje go WYLACZNIE przez `sanitizeSharedReport` (URL-e obrazkow i linkow z allowlisty hostow, CTA przepisane na sign-up), bo to tresc od anonimow serwowana pod app.appboard.dev/aso-check/r/<id>.
- **Wszystkie rynki w darmowym check-upie** (0.17.0): panel iteruje po `KEYWORD_COUNTRIES` (30 storefrontow) w przegladarce; dla Play limity proxy to 100 lookupow i 500 keyword-calli/h/IP (cache per keyword+kraj na dobe zostaje).
...
- `GET /apps/:id/audit` is **cache-first**: table `app_audits` (unique appId+country, `report` NULL while measuring). A read returns instantly with `status: "measuring" | "ready" | "failed" | "not-in-store"`; the real computation (~1 min of live iTunes calls) runs detached. **A stored report is never re-measured because a page opened**: work starts only when there is no report, when the last run died (`RUN_TIMEOUT_MS`) or failed more than an hour ago, or on `?refresh=true` (the panel's Re-check). Freshness is the scheduler's job: `AuditService.refreshAll()` every Monday 03:00 local (`isAuditRefreshDue`). A run that fails before any report exists is answered as `status: "failed"` with `error` = `app_audits.last_error` (the store client's message) and is retried on read only after an hour or on Re-check - never shown as "measuring". Never make this endpoint blocking again - the socket died at 29 s. Store score comes from the LIVE listing in the audited market's language (`appstoreMeta(id, country, iso)`), draft score from the same rules over the draft row.
- **Google Play apps get a text-only audit** (`computePlayAudit`): store text via `playstoreMeta` (`AuditService.playMeta` is the test seam), `buildAudit(app, [], { keywordsUnavailable: true })` drops every keyword rule (`KEYWORD_RULE_IDS`) because there is no difficulty data for Play; the report says `keywordsSupported: false` and the panel hides the keyword table instead of showing "you rank for nothing".
- **AI review of the audit** (`audit-ai.service.ts`): after the deterministic report, when the workspace has a working OpenRouter key, `AuditAiService.analyze` asks the model chosen for research in Settings (`AI_MODEL_RESEARCH`) for a summary, up to 4 priorities and rewrites (title/subtitle/keywords/opening, store limits enforced after the fact, only recommendable keywords). Stored in `report.ai`; a failure logs and leaves `ai: null`. The numbers never depend on the model.
- `GET /api/ai/models` returns OpenRouter's live text-model catalog (cached 6 h, `:batch` and non-text models dropped) so the Settings picker offers what exists today; `AIService.complete()` is the public completion entry point for features outside the AI module.
...
## AI prompts (single source)

`src/modules/ai/ai.prompts.ts` is the only place listing-copy prompts live: `buildListingSystemPrompt(field, mode, platform)` composes role + shared principles + the store's indexing facts + the field's rules + output rules. The runtime fallback in `resolvePrompt` (after per-app and per-workspace overrides) and the Settings "defaults" (`getAllDefaultPrompts`, native store per field) are the same text - the old duplicate `buildSystemPrompt` in `ai.service.ts` is gone. House rules baked into every prompt: no competitor or third-party names in metadata (Apple 2.3.7 / Play metadata policy), no promotional words, no emoji or typographic characters, never invent facts, count characters and cut words to fit. Keyword research (`suggestKeywords`), review replies (`draftReply`, 350-character Play limit, no incentives for ratings), localization (`translateLocalization`, `translate`), category choice and the audit review (`audit-ai.service.ts`) follow the same rules. `ai-prompts.test.ts` guards limits, policy lines and the absence of typographic characters in every prompt source file (`ai.service.ts`, `monetization.prompts.ts`, `audit-ai.service.ts`, `research.ai.ts` included).

Decisions baked into the prompts (2026-09-07 refactor, backend 0.16.3): `fieldLimit(field, platform)` is the only limit table (Play release notes 500, everything else as before) and every prompt states a limit once; the user prompt of `generateListingField` carries the current title/subtitle/keyword field of that language (draft, else remote) so the App Store once-per-word rule is checkable, and says "Current listing: unknown" instead of inviting invention; Play overrides for the description are read under `fullDescription` (`settingsField`); `generateReleaseNotes` runs in generate mode with the change list as a brief (and accepts `language`/`platform`); `suggestKeywords` returns `keywords` without the `competitors` cluster (rival names come back as `trackingOnly`, for rank tracking only) and errors on non-JSON instead of comma-splitting prose; `suggestCategory` offers `GOOGLE_PLAY_CATEGORIES` on Play (no secondary there) and rejects ids the store does not have; `stripEmoji` keeps line structure and maps typographic dashes/quotes/ellipsis to ASCII; the audit review writes summary and priorities in English and the rewrites in the listing language, treats the Play "subtitle" slot as the 80-character short description and forces `keywords: null` there.
